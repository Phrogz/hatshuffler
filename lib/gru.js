...
class Player {
	...
	static fromGRUCSV(csvObj) {
		...
		const [feet, inches] = (csvObj.Height || "5'9\"").match(/\d+/g).map(parseFloat)
...
class Season {
	...
	static fromPlayers(players, teamCount=4, roundCount=5) {
		// Sort of distribute men and women across teams based on skill using a hellaciously-inefficient algorithm
		const teams = Array.from({length:teamCount}, ()=>new Team);
		players.sortBy(p => [p.male ? 0 : 1, p.ranking()]).forEach(p => {
			// Find the team with the fewest number of players matching this player's sex
			const leastFull = teams.sortBy(t => t[p.male ? 'men' : 'women'].length)[0];
			leastFull[p.male ? 'men' : 'women'].push(p);
			if (p.baggage) leastFull[p.baggage.male ? 'men' : 'women'].push(p);
		});
	...
	swizzle() {
		...
		if (!p1.baggage && !p2.baggage) {
			...
			const b1i = t1[p1.baggage.sex].indexOf(p1.baggage),
			      b2i = t2[p2.baggage.sex].indexOf(p2.baggage);
			console.log(`Swapping ${p1} with ${p2}`);
			[t1[sex][t1i], t2[sex][t2i]] = [t2[sex][t2i], t1[sex][t1i]];
			sex = p1.baggage.sex;
			console.log(`Swapping ${p1.baggage} with ${p2.baggage}`);
			[t1[sex][b1i], t2[sex][b2i]] = [t2[sex][b2i], t1[sex][b1i]];
			...
			if (unbagged.length>0) {
				...
				console.log(`Swapping ${p1} with ${p2}`);
				[t1[sex][t1i], t2[sex][t2i]] = [t2[sex][t2i], t1[sex][t1i]];
				console.log(`Swapping ${p1.baggage} with ${b2}`);
				sex = b2.sex;
				[t1[sex][b1i], t2[sex][b2i]] = [t2[sex][b2i], t1[sex][b1i]];
			} else {
